...
namespace MarsFramework.Pages
{
    ...
    */
    ...
    class ManageListing
    {
        ...
        /// <summary>
        /// Deactivates a service from the listing with a given category and title from the excel sheet dataset
        /// </summary>
        /// <returns>
        /// If the service deactivated successfully with a true/false value
        /// </returns>
        internal bool DeActivateService()
        {
            //Populate the Excel Sheet
            ...
            if (rowNo == 0)
            {
                //Unable to find the record to deactivate
                return true;
            }
            else
            {
                IWebElement ActivateSlider = GlobalDefinitions.driver.FindElement(By.XPath("//*[@id='listing-management-section']/div[2]/div[1]/table/tbody/tr[" + rowNo + "]/td[7]/div/input"));

                if (ActivateSlider.Selected)
                {
                    ActivateSlider.Click();
                    Thread.Sleep(1000);

                    return ActivateSlider.Selected;
                }
                else
                {
                    return true;
